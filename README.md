...
| 文本类型 | 原始 UTF-8 | gzip -9 | zstd -19 | **v0.2.0 offline** | **v0.3.0 offline** | **v0.3.0 online** |
|---------|-----------|---------|----------|-------------------|-------------------|------------------|
| 中文 462 字 | 1,248 B | 781 B (0.63) | 785 B (0.63) | 679 B (0.54) | **574 B (0.46)** | **535 B (0.43)** |
| 中英混合 2,622 字 | 4,925 B | 2,954 B (0.60) | 2,903 B (0.59) | 2,640 B (0.54) | **2,464 B (0.50)** | — |
| 重复文本 200 字 | 600 B | — | — | 32 B (0.05) | **32 B (0.05)** | — |

> 括号内为压缩比（越小越好）。v0.3.0 的中文频率先验使离线模式压缩率从 0.54 提升至 **0.46**（提升 15%）。
...
- **压缩速度 13× 提升**：`CHUNK_CHARS` 从 20 提升至 200（API 调用减少 10 倍），大幅降低在线模式压缩耗时
- **解压速度 1200× 提升**：压缩时将 LLM 预测结果嵌入 .ztxt 文件（zstd 压缩存储），解压时直接读取缓存，**无需 API 调用**，解压仅需 ~1 秒
- **智能回退**：若在线模式（含缓存）生成的文件大于离线模式，自动使用离线结果，确保压缩率不降低
- **确定性保证**：预测缓存消除了 API 非确定性问题，解压结果 100% 可靠
- **格式兼容**：model_data 扩展存储 chunk_chars、max_tokens、预测缓存；旧版解码器自动忽略新增字段，v0.3.0 格式文件仍可正常解压
- 修复了 DeepSeek API 对长上下文非确定性返回导致的解压失败问题
